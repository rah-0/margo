...
`))
	...
	t.Run("invalid output stops before migrations", func(t *testing.T) {
		...
		for _, target := range []string{
			...
			if !strings.Contains(output, conf.ErrOutputPathNotDir.Error()) && !strings.Contains(output, conf.ErrOutputPathInvalid.Error()) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			wantError:    migrate.ErrVersionGap.Error(),
		...
		},
		{
			...
			wantError:    migrate.ErrInvalidSessionState.Error(),
...
`))
	t.Run("queries require an output path before migrations", func(t *testing.T) {
		...
		if !strings.Contains(output, "-outputPath is required when -queriesPath is set") {
...
func newMigrationOutputModule(t *testing.T) string {
	t.Helper()
	path := t.TempDir()
	writeTestFile(t, filepath.Join(path, "go.mod"), []byte(`module github.com/rah-0/margo/integration/migratedtest
...
func runMigrationCLI(t *testing.T, binary string, settings Settings, outputPath, migrationsPath, queriesPath string) (string, error) {
	t.Helper()
	return runMigrationCLIInDir(t, binary, settings, outputPath, migrationsPath, queriesPath, "")
}

func runMigrationCLIInDir(t *testing.T, binary string, settings Settings, outputPath, migrationsPath, queriesPath, workingDir string) (string, error) {
	t.Helper()
	args := []string{
...
import (
	...
	generated "github.com/rah-0/margo/integration/migratedtest/MargoTest"
	"github.com/rah-0/margo/integration/migratedtest/MargoTest/Users"
)
