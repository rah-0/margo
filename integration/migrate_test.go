	...
	t.Run("gaps reject the complete pending batch", func(t *testing.T) {
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				if !errors.Is(err, migrate.ErrVersionGap) {
...
`)
	...
	t.Run("connection requirements", func(t *testing.T) {
		for _, tc := range []struct {
			...
			{name: "no selected database", multiStatements: true, want: migrate.ErrDatabaseNotSelected},
			{name: "multiple statements disabled", selected: true, want: migrate.ErrMultiStatementsRequired},
		} {
			t.Run(tc.name, func(t *testing.T) {
				resetMigrationTables(t, database)
	...
	t.Run("invalid initial sessions stop before metadata", func(t *testing.T) {
		for _, tc := range []struct {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				err := migrate.Run(t.Context(), migrate.Options{DB: pool, Path: path})
				if !errors.Is(err, migrate.ErrInvalidSessionState) {
...
`)
				...
				} else if !errors.Is(err, migrate.ErrInvalidSessionState) || !errors.Is(err, migrate.ErrMigrationFailed) || !strings.Contains(err.Error(), "0002_session.sql") {
...
func assertMigrationFailure(t testing.TB, err error, filename string, code uint16) {
	t.Helper()
	if !errors.Is(err, migrate.ErrMigrationFailed) {
