...
import (
	...
	generated "github.com/rah-0/margo/tests/integration/generatedtest/MargoTest"
	"github.com/rah-0/margo/tests/integration/generatedtest/MargoTest/AllTypes"
	"github.com/rah-0/margo/tests/integration/generatedtest/MargoTest/Alpha"
	"github.com/rah-0/margo/tests/integration/generatedtest/MargoTest/Beta"
	"github.com/rah-0/margo/tests/integration/generatedtest/errs"
)
...
func TestGeneratedDatabaseNotInitialized(t *testing.T) {
	if err := generated.SetDB(nil); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
	...
	for _, begin := range []func() (*sql.Tx, error){
		generated.NewTx,
		func() (*sql.Tx, error) { return generated.NewCtxTx(t.Context()) },
		func() (*sql.Tx, error) { return generated.NewTxOpts(nil) },
		func() (*sql.Tx, error) { return generated.NewCtxTxOpts(t.Context(), nil) },
	} {
		tx, err := begin()
		if tx != nil || !errors.Is(err, errs.ErrDatabaseNotInitialized) {
			t.Fatalf("uninitialized transaction: tx=%v err=%v", tx, err)
		}
	}
}

func TestGeneratedParameterErrors(t *testing.T) {
	var entity Alpha.Entity
	for _, result := range []*Alpha.QueryResult{
		entity.DBUpdate(nil),
		entity.DBUpdateCtx(t.Context(), nil),
		entity.DBUpdateTx(nil, nil),
		entity.DBUpdateCtxTx(t.Context(), nil, nil),
		entity.DBUpdate(Alpha.NewQueryParams().WithWhere(Alpha.FieldUuid)),
		entity.DBUpdate(Alpha.NewQueryParams().WithUpdate(Alpha.FieldAnimal)),
	} {
		if !errors.Is(result.Error, errs.ErrUpdateParamsRequired) {
			t.Fatalf("missing update fields: %v", result.Error)
		}
	}
	for _, result := range []*Alpha.QueryResult{
		entity.DBExists(nil),
		entity.DBExistsCtx(t.Context(), nil),
		entity.DBExistsTx(nil, nil),
		entity.DBExistsCtxTx(t.Context(), nil, nil),
	} {
		if result.Exists || !errors.Is(result.Error, errs.ErrExistsParamsRequired) {
			t.Fatalf("missing existence-query params: exists=%t err=%v", result.Exists, result.Error)
		}
	}
}

func TestGeneratedMultipleRows(t *testing.T) {
	resetTables(t)
	for _, animal := range []string{"cat", "dog"} {
		entity := Alpha.Entity{Uuid: testUUID(), Animal: animal}
		if result := entity.DBInsert(Alpha.NewQueryParams().WithInsert(Alpha.FieldUuid, Alpha.FieldAnimal)); result.Error != nil {
			t.Fatal(result.Error)
		}
	}
	for _, result := range []*Alpha.QueryResult{
		Alpha.QuerySingleAnimal(),
		Alpha.QuerySingleAnimalCtx(t.Context()),
		Alpha.QuerySingleAnimalTx(nil),
		Alpha.QuerySingleAnimalCtxTx(t.Context(), nil),
	} {
		if result.Exists || !errors.Is(result.Error, errs.ErrMultipleRows) {
			t.Fatalf("single-row query: exists=%t err=%v", result.Exists, result.Error)
		}
	}
}
...
func TestGeneratedMissingReturns(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "MissingReturnsOne", mode: "one", errs: []error{
			...
			generated.QueryMissingReturnsOneTx(nil).Error,
			generated.QueryMissingReturnsOneCtxTx(t.Context(), nil).Error,
		}},
		{name: "MissingReturnsMany", mode: "many", errs: []error{
			generated.QueryMissingReturnsMany().Error,
			generated.QueryMissingReturnsManyCtx(t.Context()).Error,
			generated.QueryMissingReturnsManyTx(nil).Error,
			generated.QueryMissingReturnsManyCtxTx(t.Context(), nil).Error,
		}},
	} {
		for _, err := range test.errs {
			if !errors.Is(err, errs.ErrMissingReturns) || !strings.Contains(err.Error(), test.name) || !strings.Contains(err.Error(), "ResultMode="+test.mode) {
				t.Fatalf("named query %s: expected metadata sentinel and query context, got %v", test.name, err)
			}
		}
	}
}
