...
func margoRepositoryDir(t testing.TB) string {
	...
	if callerOK && filepath.IsAbs(sourceFile) {
		if repositoryDir := findMargoRepository(filepath.Dir(sourceFile)); repositoryDir != "" {
	...
	if repositoryDir := findMargoRepository(workingDir); repositoryDir != "" {
		return repositoryDir
	}
...
func findMargoRepository(dir string) string {
	for {
