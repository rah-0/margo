...
func TestDiscoverInvalidFilename(t *testing.T) {
	for _, name := range []string{
		...
		t.Run(name, func(t *testing.T) {
			...
			writeMigration(t, dir, name)
			_, err := discover(dir)
			if !errors.Is(err, ErrInvalidFilename) || !strings.Contains(err.Error(), name) {
...
func TestDiscoverDuplicateVersion(t *testing.T) {
	...
	if _, err := discover(dir); !errors.Is(err, ErrDuplicateVersion) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, version := range tt.versions {
				migrations = append(migrations, Migration{Version: version, Path: fmt.Sprintf("%04d_change.sql", version)})
			}
			pending, err := pendingMigrations(migrations, tt.current)
			if tt.missing != 0 {
				if !errors.Is(err, ErrVersionGap) || !strings.Contains(err.Error(), fmt.Sprintf("expected %d ", tt.missing)) {
