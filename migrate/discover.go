...
// Discover validates SQL migration filenames directly inside source's root
// directory (".") and returns relative names sorted by increasing version.
// It ignores non-SQL files and subdirectories, rejects zero or duplicate versions,
// and permits numbering gaps. PendingMigrations validates the pending sequence
// against a database's current version. A nil source returns an error matching
// fs.ErrInvalid. The caller retains ownership of source; discovery only reads its
// directory.
func Discover(source fs.FS) ([]Migration, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read SQL directory %q: %w", ".", err)
	}
	...
	for _, entry := range entries {
		...
		if matches == nil {
			return nil, fmt.Errorf("%w: %q", errs.ErrInvalidFilename, filename)
		}
		version, err := strconv.ParseUint(matches[1], 10, 64)
		if err != nil {
			return nil, fmt.Errorf("%w: %q: %w", errs.ErrInvalidFilename, filename, err)
		}
		if version == 0 {
			return nil, fmt.Errorf("%w: %q: version must be positive", errs.ErrInvalidFilename, filename)
		}
		if previous, exists := versions[version]; exists {
			return nil, fmt.Errorf("%w: %d in %q and %q", errs.ErrDuplicateVersion, version, previous, filename)
		}
...
// sorted by increasing version, as returned by Discover. The returned suffix shares
// the input slice's backing array; this function does not modify it.
